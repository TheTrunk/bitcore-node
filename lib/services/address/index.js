...
AddressService.prototype.getAddressSummary = function(address, options, callback) {
  ...
  self.getAddressHistory(address, options, function(err, results) {
    ...
    result.balance = Unit.fromSatoshis(result.balanceSat).toHUSH();
    result.totalReceived = Unit.fromSatoshis(result.totalReceivedSat).toHUSH();
    result.totalSent = Unit.fromSatoshis(result.totalSentSat).toHUSH();
    result.unconfirmedBalance = Unit.fromSatoshis(result.unconfirmedBalanceSat).toHUSH();
    ...
    function(next) {
      ...
      utxoStream.on('data', function(data) {
        ...
        results.push({
          ...
          amount: Unit.fromSatoshis(value.satoshis).toHUSH(),
          height: value.height,
...
AddressService.prototype._getMempoolUtxos = function(tx, address) {
  ...
  for(var i = 0; i < tx.outputs.length; i++) {
    ...
    results.push({
      ...
      amount: Unit.fromSatoshis(output.value).toHUSH(),
      height: null,
