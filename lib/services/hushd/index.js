...
/**
 * Provides a friendly event driven API to hushd in Node.js. Manages starting and
 * stopping hushd as a child process for application support, as well as connecting
 * to multiple hushd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Hush(options) {
  if (!(this instanceof Hush)) {
    return new Hush(options);
  }
...
util.inherits(Hush, Service);

Hush.dependencies = [];

Hush.DEFAULT_MAX_TXIDS = 1000;
Hush.DEFAULT_MAX_HISTORY = 50;
Hush.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Hush.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Hush.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Hush.DEFAULT_SPAWN_RESTART_TIME = 5000;
Hush.DEFAULT_SPAWN_STOP_TIME = 10000;
Hush.DEFAULT_TRY_ALL_INTERVAL = 1000;
Hush.DEFAULT_REINDEX_INTERVAL = 10000;
Hush.DEFAULT_START_RETRY_INTERVAL = 5000;
Hush.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Hush.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Hush.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Hush.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Hush.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Hush.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Hush.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Hush.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Hush.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Hush.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Hush.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Hush.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Hush.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Hush.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Hush.prototype._initCaches = function() {
...
Hush.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Hush.prototype.getAPIMethods = function() {
  var methods = [
...
Hush.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'hushd/' + name, 'total:', this.subscriptions[name].length);
};

Hush.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'hushd/' + name, 'total:', this.subscriptions[name].length);
};

Hush.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'hushd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Hush.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'hushd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Hush.prototype.unsubscribeAddressAll = function(emitter) {
...
Hush.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Hush.DEFAULT_CONFIG_SETTINGS;
...
Hush.prototype._parseHushConf = function(configPath) {
  var options = {};
...
Hush.prototype._expandRelativeDatadir = function() {
...
Hush.prototype._loadSpawnConfiguration = function(node) {
  ...
  _.extend(this.spawn.config, this._parseHushConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseHushConf(networkConfigPath));
  }
...
Hush.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
...
Hush.prototype._resetCaches = function() {
...
Hush.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Hush.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Hush.prototype._initChain = function(callback) {
  var self = this;
...
Hush.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8822
  };
...
Hush.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
...
Hush.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Hush.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('hushd/hashblock', message.toString('hex'));
    }
  }

};

Hush.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Hush.prototype._updateTip = function(node, message) {
  var self = this;
...
Hush.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Hush.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('hushd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Hush.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('hushd/rawtransaction', message.toString('hex'));
    }
...
Hush.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Hush.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Hush.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Hush.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Hush.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    ...
    }, node._reindexWait || Hush.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Hush.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Hush.prototype._stopSpawnedHush = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
...
Hush.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedHush(function(err) {
    if (err) {
      return callback(err);
    }
...
Hush.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
Hush.prototype.start = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Hush.prototype.syncPercentage = function(callback) {
  var self = this;
...
Hush.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
Hush.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Hush.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Hush.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Hush.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
Hush.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Hush.prototype._getConfirmationsDetail = function(transaction) {
...
Hush.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Hush.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Hush.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A Hush transaction id
 * @param {Function} callback
 */
Hush.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Hush.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Hush.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Hush.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Hush.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Hush.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Hush.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.getInfo = function(callback) {
  var self = this;
...
Hush.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hush.prototype.stop = function(callback) {
