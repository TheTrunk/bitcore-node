...
/**
 * Will return the path and default bitcore-node-hush configuration. It will search for the
...
function getDefaultConfig(options) {
  ...
  var defaultConfigFile = path.resolve(defaultPath, './bitcore-node-hush.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    var now = new Date();
    // bitcore-node-hush.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `bitcore-node-hush.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
  ...
  var defaultConfig = {
    ...
    servicesConfig: {
      'insight-api-hush': {
        cwdRequirePath: 'node_modules/insight-api-hush'
      },
      'insight-ui-hush': {
        cwdRequirePath: 'node_modules/insight-ui-hush'
      },
      'bitcore-wallet-service-hush': {
          cwdRequirePath: 'node_modules/bitcore-wallet-service-hush'
      },
	  'hushd': {
          'spawn': {
            datadir: './data',
            exec: '/home/tj/hush/src/hushd'
          }
        }
    }
  };
